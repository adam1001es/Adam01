...
import { getKontingent, istZahlendesKonto, formatArbeitsblaetterSpanne, zuCoins } from "@/lib/quota";
...
export async function POST(request: NextRequest) {
  ...
  if (kontingent.verbleibend <= 0) {
    const grund = kontingent.tier
      ? `Dein Guthaben für diesen Zyklus (${zuCoins(kontingent.limit)} Coins, ${formatArbeitsblaetterSpanne(kontingent.limit)}) ist aufgebraucht. Neuer Zyklus ab ${kontingent.zyklusEnde.toLocaleDateString("de-AT")}.`
      : `Dein einmaliges kostenloses Guthaben (${zuCoins(kontingent.limit)} Coins, ${formatArbeitsblaetterSpanne(kontingent.limit)}) ist aufgebraucht. Für mehr: ein Abo bei der Person anfragen, die den Zugang verwaltet.`;
