...
/** Umschaltet das "bearbeitet"-Flag einer Forum-Meldung, analog zu
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const target = await prisma.forumMeldung.findUnique({ where: { id: params.id } });
  if (!target) {
    return NextResponse.json({ error: "Meldung nicht gefunden." }, { status: 404 });
  }

  const updated = await prisma.forumMeldung.update({
    where: { id: params.id },
