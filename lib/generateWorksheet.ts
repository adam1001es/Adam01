...
Bei "reihenfolge" MUSS "reihenfolgeElemente" gesetzt sein: 3-6 kurze Ereignisse/Schritte in der RICHTIGEN chronologischen bzw. logischen Reihenfolge (das System mischt sie selbst für den Druck - du musst dich nicht um eine "zufällige" Anordnung kümmern, liefere sie einfach korrekt geordnet). "frage" ist die Arbeitsanweisung (z.B. "Bringe die Ereignisse in die richtige Reihenfolge, indem du die Zahlen 1-4 daneben einträgst."). "loesung" nennt die korrekte Reihenfolge in Klartext (z.B. "1. ..., 2. ..., 3. ..."). WICHTIG: Schüler:innen müssen die Reihenfolge beim Bearbeiten tatsächlich HERLEITEN können, nicht nur auswendig kennen oder raten. Ist sie NICHT aus echtem Allgemeinwissen oder zwingender Alltagslogik ableitbar (z.B. "erst Schuhe anziehen, dann binden"), sondern hängt von konkreten Fakten ab, die eigens vermittelt werden müssten (Abläufe, Daten, Fachbegriffe, Detailwissen) - dann MUSS zusätzlich "lesetext" gesetzt werden: ein kurzer Sachtext (ca. 3-8 Sätze), der alle für DIESE Aufgabe nötigen Informationen enthält, sodass sich die Reihenfolge beim Lesen erschließt statt vorausgesetzt zu werden. Ein Arbeitsblatt mit mehreren "reihenfolge"-Aufgaben zu unterschiedlichen, spezifischen Abläufen braucht so gut wie immer einen eigenen "lesetext" pro Aufgabe - eine einzelne allgemeine "einleitung" für das ganze Arbeitsblatt reicht dafür NICHT aus.
